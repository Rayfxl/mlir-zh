...
Passes是转换和优化的基础架构。本文档概述了 MLIR 中的pass基础设施及其使用方法。

有关 MLIR 及其核心方面（如 IR 结构和操作）的更多信息，请参阅 [MLIR 规范](https://mlir.llvm.org/docs/LangRef/)。

有关 MLIR 中图重写的快速入门，请参阅 [MLIR 重写](https://mlir.llvm.org/docs/Tutorials/QuickstartRewrites/)。如果转换涉及模式匹配操作 DAG，这是一个很好的开始。

## 操作 Pass

在 MLIR 中，抽象和转换的主要单位是一个[操作](https://mlir.llvm.org/docs/LangRef/#operations)。因此，pass管理器被设计用于处理不同嵌套级别的操作实例。在下面的段落中，我们将一个pass作用于的那个操作称为“当前操作”。

[pass manager](https://mlir.llvm.org/docs/PassManagement/#pass-manager) 的结构以及嵌套的概念将在下面进一步详细介绍。MLIR中的所有passes都派生于`OperationPass` ，并遵守以下限制；任何不遵守限制的行为都会在多线程和其他高级场景中导致问题：
...
默认情况下，操作pass是操作无关的，这意味着它是对添加到pass管理器中的操作类型进行操作。这意味着pass可以对多种不同类型的操作进行操作。在编写与操作无关的pass时，不应对其运行的操作进行假设。这类pass的例子包括[规范化](https://mlir.llvm.org/docs/Passes/#-canonicalize)和[通用子表达式消除](https://mlir.llvm.org/docs/Passes/#-cse)。
...
// 只有直接用 C++ 定义的passes才需要这样做。声明式定义的passes使用更简洁的机制来提供这些实用工具。
...
如果pass需要将其执行限制在特定类型或类别的操作上，则可以在其顶部应用额外的过滤功能。这样就能将之前操作无关的pass转换为更适合特定上下文的pass。有多种方法可以过滤pass的执行，并且可以在不同的上下文中应用过滤：
...
当带有静态过滤功能的pass被添加到[`特定操作`的pass管理器](https://mlir.llvm.org/docs/PassManagement/#oppassmanager)中时，它会断言 pass管理器中的操作类型满足该pass的静态约束。当添加到[`操作无关`的pass管理器](https://mlir.llvm.org/docs/PassManagement/#oppassmanager)时，该pass管理器及其包含的所有passes都将继承该pass的静态约束。例如，如果pass对`FunctionOpInterface`进行过滤，如上面的 `MyFunctionPass` 示例所示，那么在执行pass管理器中的任何passes时，将只考虑实现 `FunctionOpInterface` 的操作。我们必须牢记这一不变量，因为添加到操作无关的pass管理器的每个pass都会进一步限制可在其上调度的操作。请看下面的示例：
...
必须先在 MLIRContext 中加载方言，然后才能创建这些方言中的实体（操作、类型、属性等）。在开始执行多线程pass管道之前，也必须加载这些方言。为此，可能从不能保证已被加载的方言中创建实体的pass必须通过重写  `getDependentDialects()`方法并显式声明此方言列表来表达这一点。另请参阅 [TableGen 规范](https://mlir.llvm.org/docs/PassManagement/#tablegen-specification)中的 `dependentDialects` 字段。

### 初始化

在某些情况下，Pass可能包含动态构造的状态，但在 Pass 的连续运行中，重新计算那些状态的代价可能很高。其中一个例子就是使用[基于 `PDL`](https://mlir.llvm.org/docs/Dialects/PDLOps/) 的模式时，这些[模式](https://mlir.llvm.org/docs/PatternRewriter/)在运行时被编译为字节码。在这些情况下，pass可以重写下面的钩子来初始化这种重的状态：
...
与转换passes一起的一个重要概念是分析。从概念上讲，分析与转换passes类似，不同之处在于它们只计算特定操作的信息，而不对其进行修改。在 MLIR 中，分析不是passes，而是独立的类，它们按需延迟计算并缓存，以避免不必要的重新计算。MLIR 中的分析必须遵守以下规定：
...
```c++
void MyOperationPass::runOnOperation() {
  // 将所有分析标记为保留。如果pass能保证没有执行转换，这一点就非常有用。
  markAllAnalysesPreserved();

  // 将特定分析标记为保留。如果执行了某些转换，但某些分析未受影响或被明确保留，则使用此方法。
...
`OpPassManager`本质上是一组passes，它们被锚定在给定嵌套级别上的操作执行。pass 管理器可以是`特定操作的`（锚定在特定的操作类型上），也可以是 `操作无关的`（不限于任何特定操作，并在任何可行的操作类型上执行）。锚定pass管理器的操作类型必须符合以下要求：

- 必须注册并标记为[`IsolatedFromAbove`](https://mlir.llvm.org/docs/Traits/#isolatedfromabove)。
  - passes不应修改正在处理的当前操作之上的操作。如果操作未被隔离，它可能会无意中修改或遍历它不该如此做的操作的 SSA 使用列表。

可通过 `addPass` 向pass管理器添加passes。

`OpPassManager` 通常是通过 `nest<OpT>` 或 `nestAny` 方法将管道显式嵌套在另一个存在的 `OpPassManager` 中来创建的。前一种方法接受嵌套的pass管理器将操作的操作类型。后一种方法嵌套了一个`操作无关的` pass 管理器，它可以在任何可行的操作类型上运行。从这个意义上说，这对应于IR[区域](https://mlir.llvm.org/docs/LangRef/#regions)内的[结构](https://mlir.llvm.org/docs/Tutorials/UnderstandingTheIRStructure/)嵌套。
...
在某些情况下，在另一个pass中运行一个pass管道可能是有用的，这样就可以根据正在运行的当前操作的一些不变量进行配置或过滤。例如，[Inliner Pass](https://mlir.llvm.org/docs/Passes/#-inline)可能希望在内联时运行程序内简化passes，以生成更好的代价模型，并提供更优化的内联。要启用此功能，passes可以通过`LogicalResult Pass::runPipeline(OpPassManager &, Operation *)`方法对正在操作的当前操作或嵌套在当前操作中的任何操作运行任意 `OpPassManager`。此方法返回动态管道是成功还是失败，类似于顶层 `PassManager：：run` 方法的结果。一个简单的示例如下所示：
...
每当pass管道应以嵌套方式运行时，即当嵌套管道无法与主pass管道的其他部分一起静态调度时，应使用本节所述的机制。更具体地说，一个`PassManager`通常不需要在一个`Pass`中构造。使用 `runPipeline` 还能确保所有分析、[插桩](https://mlir.llvm.org/docs/PassManagement/#pass-instrumentation)和其他与pass管理器相关的组件都与正在执行的动态管道集成在一起。
...
在各种pass类型的示例定义中简要显示了 `PassRegistration` 类。该机制允许注册pass类，以便可以在[文本pass管道描述](https://mlir.llvm.org/docs/PassManagement/#textual-pass-pipeline-specification)中创建它们。下面是一个注册示例：
...
注意：请确保pass是可拷贝构造的，且这种方式不会共享数据，因为[pass管理器](https://mlir.llvm.org/docs/PassManagement/#pass-manager)可能会创建pass的副本来并行执行。
...
前面几节详细介绍了如何使用特定参数和描述来注册passes和pass管道。一旦注册成功，就可以通过字符串描述来配置pass管理器。这对于像`mlir-opt`这种通过命令行配置pass管理器的工具，或者作为使用[动态pass管道](https://mlir.llvm.org/docs/PassManagement/#dynamic-pass-pipelines)的passes选项，尤其有用。
...
- `op-anchor`
  - 这对应于锚定pass管理器执行的助记符名称。该名称可以是运行passes的操作名称，例如 `func.func` 或 `builtin.module`，也可以是任何操作名称，这是对于在任何可行操作（即任何可用于锚定pass管理器的操作）上执行的操作无关的pass管理器来说的。
- `pass-name` | `pass-pipeline-name`
  - 这对应于已注册的pass或pass管道的参数，例如 `cse` 或 `canonicalize`。
- `options`
  - 选项是特定的键值对，表示由pass或pass管道定义的选项，正如[“特定实例Pass选项”](https://mlir.llvm.org/docs/PassManagement/#instance-specific-pass-options)部分所述。有关文本管道中的示例用法，请参阅此部分。
...
Pass的某些方面可以以声明方式指定，其形式与[操作](https://mlir.llvm.org/docs/DefiningDialects/Operations/)类似。这种规范简化了定义passes时使用的几种机制。它可用于生成pass注册调用、定义样板pass实用工具和生成pass文档。
...
使用 `gen-pass-doc` 生成器，可以为每个pass生成 markdown 文档。请参阅 [Passes.md](https://mlir.llvm.org/docs/Passes/)，了解实际 MLIR passes的输出示例。
...
- `dependentDialects`
  - 一个字符串列表，表示pass可能引入实体、属性/操作/类型/等的`方言`类。
...
PassInstrumentation实例可以通过 `addInstrumentation` 方法直接注册到 [PassManager](https://mlir.llvm.org/docs/PassManagement/#pass-manager) 实例。添加到 PassManager的插桩代码以类似堆栈的方式运行，即最后一个执行 `runBefore*` 钩子的插桩工具将是第一个执行相应 `runAfter*` 钩子的插桩工具。`PassInstrumentation` 类的钩子保证以线程安全的方式执行，因此不需要额外的同步。下面是一个示例插桩代码，该插桩代码计算了 `DominanceInfo` 分析的次数：
...
MLIR 中的[pass管理器](https://mlir.llvm.org/docs/PassManagement/#pass-manager)包含一个内置机制，用于在发生崩溃或[pass失败](https://mlir.llvm.org/docs/PassManagement/#pass-failure)时生成可重现数据。该功能可以通过 `PassManager::enableCrashReproducerGeneration` 方法或命令行标志 `mlir-pass-pipeline-crash-reproducer` 启用。在这两种情况下，都需要提供一个参数，该参数对应于应将可重现数据写入的输出 `.mlir` 文件名。重现器包含正在执行的pass管理器的配置，以及运行任何passes之前的初始 IR。重现器作为外部资源存储在装配格式中。潜在的重现器可能具有以下形式：
