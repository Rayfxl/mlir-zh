...
- [Benefits](https://mlir.llvm.org/docs/DeclarativeRewrites/#benefits)
- [Strengths and Limitations](https://mlir.llvm.org/docs/DeclarativeRewrites/#strengths-and-limitations)
- Rule Definition
  - [Source pattern](https://mlir.llvm.org/docs/DeclarativeRewrites/#source-pattern)
  - [Result pattern](https://mlir.llvm.org/docs/DeclarativeRewrites/#result-pattern)
  - [Supporting auxiliary ops](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-auxiliary-ops)
  - [Supporting multi-result ops](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-multi-result-ops)
  - [Supporting variadic ops](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-variadic-ops)
  - [Supplying additional constraints](https://mlir.llvm.org/docs/DeclarativeRewrites/#supplying-additional-constraints)
  - [Supplying additional result patterns](https://mlir.llvm.org/docs/DeclarativeRewrites/#supplying-additional-result-patterns)
  - [Adjusting benefits](https://mlir.llvm.org/docs/DeclarativeRewrites/#adjusting-benefits)
- Rewrite directives
...
- Debugging Tips
  - [Run `mlir-tblgen` to see the generated content](https://mlir.llvm.org/docs/DeclarativeRewrites/#run-mlir-tblgen-to-see-the-generated-content)
  - [Compilation error: no matching member function for call to ‘build’](https://mlir.llvm.org/docs/DeclarativeRewrites/#compilation-error-no-matching-member-function-for-call-to-build)
...
- [`NativeCodeCall`](https://mlir.llvm.org/docs/DeclarativeRewrites/#nativecodecall-transforming-the-generated-op) returning more than one results.

## Rule Definition [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#rule-definition)
...
We allow multiple result patterns to support [multi-result ops](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-multi-result-ops) and [auxiliary ops](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-auxiliary-ops), but frequently we just want to convert one DAG of operations to another DAG of operations. There is a handy wrapper of `Pattern`, `Pat`, which takes a single result pattern:
...
#### Binding op arguments and limiting the match [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#binding-op-arguments-and-limiting-the-match)
...
#### Matching DAG of operations [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#matching-dag-of-operations)
...
#### Binding op results [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#binding-op-results)
...
The above will bind `$b_result` to the matched `BOp`’s result. (There are more details regarding multi-result ops, which is covered [later](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-multi-result-ops).)
...
#### Referencing bound symbols [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#referencing-bound-symbols)
...
#### Building operations [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#building-operations)
...
#### Generating DAG of operations [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#generating-dag-of-operations)
...
#### Binding op results [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#binding-op-results-1)
...
#### `NativeCodeCall`: transforming the generated op [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#nativecodecall-transforming-the-generated-op)
...
##### `NativeCodeCall` placeholders [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#nativecodecall-placeholders)
...
##### `NativeCodeCall` binding multi-results [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#nativecodecall-binding-multi-results)

To bind multi-results and access the N-th result with `$<name>__N`, specify the number of return values in the template. Note that only `Value` type is supported for multiple results binding. For example,
...
##### Customizing entire op building [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#customizing-entire-op-building)
...
#### `__N` suffix [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#__n-suffix)

The `__N` suffix is specifying the `N`-th result as a whole (which can be [variadic](https://mlir.llvm.org/docs/DeclarativeRewrites/#supporting-variadic-ops)). For example, we can bind a symbol to some multi-result op and reference a specific result later:
...
#### Replacing multi-result ops [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#replacing-multi-result-ops)

The above example also shows how to replace a matched multi-result op.

To replace an `N`-result op, the result patterns must generate at least `N` declared values (see [Declared vs. actual value](https://mlir.llvm.org/docs/DeclarativeRewrites/#declared-vs-actual-value) for definition). If there are more than `N` declared values generated, only the last `N` declared values will be used to replace the matched op. Note that because of the existence of multi-result op, one result pattern **may** generate multiple declared values. So it means we do not necessarily need `N` result patterns to replace an `N`-result op. For example, to replace an op with three results, you can have
...
#### Declared vs. actual value [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#declared-vs-actual-value)
...
#### Match variadic operand [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#match-variadic-operand)
...
## Rewrite directives [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#rewrite-directives)
...
## Debugging Tips [¶](https://mlir.llvm.org/docs/DeclarativeRewrites/#debugging-tips)
...
This is because DRR is failing to call a `build()` method with result type deduction ability. See [building operations](https://mlir.llvm.org/docs/DeclarativeRewrites/#building-operations) for more details.